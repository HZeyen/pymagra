...
class Dialog(QtWidgets.QDialog):
    ...
    Input:
        ...
        values (list, string, float or int): initial values for LineEdit fields,
                            number of radiobutton activated by default (natural counting).
    ...
    def __init__(self, labels, types, values=None, title="Title"):
        ...
        for i, l in enumerate(labels):
            ...
            if types[i].lower() == "l":
                if values[i]:
                    if values[i].lower() == "b":
                        self.dlabels.append(QtWidgets.QLabel("<b>" + l + "</b>"))
                    elif values[i].lower() == "i":
                        self.dlabels.append(QtWidgets.QLabel("<i>" + l + "</i>"))
                    else:
                        self.dlabels.append(QtWidgets.QLabel(l))
                else:
                    self.dlabels.append(QtWidgets.QLabel(l))
                ...
                rck = int(values[i]) - 1
                if rck < 0 or rck >= len(l):
                    rck = 0
                for ir, l in enumerate(l):
