...
def median_plot(data, median1_even, median1_odd, median2_even, median2_odd,
    ...
    ax_median.plot(
        nline_odd, median1_odd, color="c", marker="*",
...
def spector2D_plot(data, depths1, intercepts1, depths2, intercepts2, xpos,
    ...
    while True:
        ...
        kkk2 = kk2[max_pos2]
        depth1, intercept1 = spectrum_plot(
            dd, kk, xpos[ixclick], depth1, depth2, intercept1, intercept2,
            d, kkk, dd2, kk2, ypos[iyclick], d2, kkk2)
...
def plot_gradients(data_c, color="rainbow"):
    ...
    ax_grad = fig_grad.plot_images(
        ...
        ylabel=[f"Northing [{dunit}]", f"Northing [{dunit}]",
                f"Northing [{dunit}]"],
        clabel=[f"Vert. grad. [{unit}/m]", f"2nd vert. grad. [{unit}/m2]",
                f"Hor. grad. [{unit}/m]"], c=color, percent=0.005)
...
def plot_tilt(data, color="rainbow"):
    ...
    ax_tilt_ang = fig_tilt_ang.plot_images(
        ...
        ptitle=f"{data.title}\nTilt angle", xlabel=f"Easting [{dunit}]",
        ylabel=f"Northing [{dunit}]", clabel="Tilt_angle [rad]", c=color)
    if data.geography_flag:
...
# If line corresponds to clicked one, plot analytic signal
            if ll == nline:
                ...
                ax_sig.plot(
                    pos[d_interest > 0], d_interest[d_interest > 0], "g*",
...
# division by 0)
                y = 1./(d_interest[n1:n2]+d_interest[p]*1.e-20)
...
# Plot colorbar for depths into the axis
        ...
        dbar = fig_ana.fig.figure.colorbar(
            gna, cax=cax, extend="max", orientation="vertical", anchor=(0, 0),
            shrink=0.5, aspect=5)
