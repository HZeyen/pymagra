...
class Geometrics:
    ...
    def append_data(self, x, y, d, year, month, day, hour, minute, second,
        ...
        self.minute = np.concatenate((self.minute,
                                      np.array(minute, dtype=int)))
        self.second = np.concatenate((self.second,
...
# In addition, times are stored
        ...
        if first:
            if abs(self.x[0]-self.x[self.segments[0]["mark_samples"][-1]]) >\
                    abs(self.y[0] -
                        self.y[self.segments[0]["mark_samples"][-1]]):
                self.direction = 1
