...
# corrections or others.
    ...
    def read_txt(self, file, height1=None, height2=None, dec=None, title=None):
        ...
        for i in range(nx):
            ...
            self.segments[line]["median2"] = np.nanmedian(
                self.sensor2_inter[n1:n2])
...
# If user wand to change setting, go béck to beginning of while loop
        ...
        if self.grad_data:
            ...
            (self.sensor1_inter, _, _, self.x_inter, self.y_inter,
             self.z_inter, self.t_inter, self.topo_inter) =\
                u.interpol_2D(self, dx=self.dx, dy=self.dy,
                              fill_hole=self.hole_flag)
        ...
        if self.nan_flag:
            ...
            if self.grad_data:
                ...
                self.sensor2_inter = u.extrapolate(
                    self.sensor2_inter, self.x_inter, self.y_inter)
                self.grad_inter = (
                    self.sensor2_inter - self.sensor1_inter) / self.d_sensor
            self.treatments["nan_fill"] = True
        else:
    ...
    def reduce_pole(self):
        ...
        if self.grad_data:
            ...
            self.grad_inter = (
                self.sensor2_inter - self.sensor1_inter) / self.d_sensor
            self.grad_fill = (
                self.sensor1_fill - self.sensor2_fill) / self.d_sensor
